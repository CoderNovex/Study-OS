...
let streak=JSON.parse(localStorage.getItem('studyOS_streak')||'null')||{count:0,lastDate:null};
let daily=JSON.parse(localStorage.getItem('studyOS_daily')||'null')||{date:today(),tasks:0,focusSeconds:0};
function today(){const d=new Date();return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
function ensureToday(){const d=today();if(daily.date!==d)daily={date:d,tasks:0,focusSeconds:0}}
function save(){localStorage.setItem('studyOS_tasks',JSON.stringify(tasks));localStorage.setItem('studyOS_focus',String(seconds));localStorage.setItem('studyOS_streak',JSON.stringify(streak));localStorage.setItem('studyOS_daily',JSON.stringify(daily))}
...
function recordActivity(){const d=today();if(streak.lastDate===d)return;if(streak.lastDate&&daysBetween(streak.lastDate,d)===1)streak.count++;else streak.count=1;streak.lastDate=d;save();renderStreak()}
function renderStreak(){$('#streak').textContent=`${streak.count} day${streak.count===1?'':'s'}`}
function renderDaily(){ensureToday();const taskPct=Math.min(100,(daily.tasks/5)*100),focusPct=Math.min(100,(daily.focusSeconds/3600)*100),score=Math.round((taskPct+focusPct)/2);$('#dailyTasks').textContent=`${daily.tasks} / 5`;$('#dailyFocus').textContent=`${Math.floor(daily.focusSeconds/60)} / 60 min`;$('#dailyLabel').textContent=`${score}%`;$('#dailyBar').style.width=`${score}%`;$('#dailyGoal').textContent=score>=100?'🎉 Goal complete!':score>=50?'🔥 Halfway there!':'Keep going!';$('#progress').textContent=`${score}%`}
function render(){ensureToday();const box=$('#tasks');box.innerHTML='';tasks.forEach((t,i)=>{const row=document.createElement('label');row.className='task '+(t.done?'done':'');row.innerHTML=`<input type="checkbox" ${t.done?'checked':''}><span>${t.text}</span><button class="delete-task" type="button" title="Delete task" aria-label="Delete ${t.text}">🗑️</button>`;row.querySelector('input').onchange=e=>{t.done=e.target.checked;if(t.done){daily.tasks++;recordActivity()}save();render()};row.querySelector('.delete-task').onclick=()=>{tasks.splice(i,1);save();render()};box.appendChild(row)});const done=tasks.filter(t=>t.done).length;$('#taskCount').textContent=`${done} / ${tasks.length}`;$('#focusTime').textContent=formatTime(seconds);renderStreak();renderDaily();save()}
function tick(){seconds++;ensureToday();daily.focusSeconds++;render()}
function updatePomodoro(){const display=$('#pomodoroTime');if(display)display.textContent=formatTime(pomodoroSeconds)}
function endPomodoro(){clearInterval(pomodoroTimer);pomodoroTimer=null;pomodoroRunning=false;$('#pomodoroBtn').textContent='Start 25 min';ensureToday();daily.focusSeconds+=25*60;recordActivity();save();renderDaily();document.title='Study-OS';if('Notification'in window&&Notification.permission==='granted')new Notification('Study-OS',{body:'Pomodoro complete! Take a short break. 🎉'});alert('Pomodoro complete! 🎉 Take a short break.')}
