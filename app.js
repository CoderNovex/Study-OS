...
function save(){localStorage.setItem('studyOS_tasks',JSON.stringify(tasks));localStorage.setItem('studyOS_focus',String(seconds));localStorage.setItem('studyOS_streak',JSON.stringify(streak));}
...
function endPomodoro(){clearInterval(pomodoroTimer);pomodoroTimer=null;pomodoroRunning=false;$('#pomodoroBtn').textContent='Start 25 min';recordActivity();document.title='Study-OS';if('Notification'in window&&Notification.permission==='granted')new Notification('Study-OS',{body:'Pomodoro complete! Take a short break. 🎉'});alert('Pomodoro complete! 🎉 Take a short break.');}
